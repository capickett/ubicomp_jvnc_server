 ...
 * Finally the data follows in the form [bgpixel][subrect][subrect]... where
 * each [subrect] is [pixel][rfbCoRRERectangle]. This means that the whole
 * rectangle must be at most 255x255 pixels.
 **/
...
public class CoRREStub extends Rect {
    ...
    public CoRREStub(int[] pixels, PixelFormat pixelFormat, int offsetX,
        ...
        if ((w <= 0xFF) && (h <= 0xFF)) {
            rect = new CoRRE(pixels, pixelFormat, offsetX, offsetY, scanline,
                    x, y, w, h);
            ...
            for (currentY = 0; currentY < h; currentY += 0xFF) {
                for (currentX = 0; currentX < w; currentX += 0xFF) {
                    ...
                    rect = new CoRRE(pixels, pixelFormat, offsetX, offsetY,
                            scanline, x + currentX, y + currentY, currentW,
                            currentH);
                    vector.addElement(rect);
                }
            }
