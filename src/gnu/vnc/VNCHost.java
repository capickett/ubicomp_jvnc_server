    ...
    public static void main(String args[]) {
        ...
        for (Enumeration<String> e = properties.getKeys(); e.hasMoreElements();) {
            ...
            if (key.endsWith(".class")) {
                ...
                width = Integer.parseInt(properties.getString(display
                        + ".width"));
                height = Integer.parseInt(properties.getString(display
                        + ".height"));
                try {
                ...
                try {
                    restrictedTo = properties.getString(display
                            + ".restrictedTo");
                } catch (MissingResourceException x) {
                }
                try {
                    noPasswordFor = properties.getString(display
                            + ".noPasswordFor");
                } catch (MissingResourceException x) {
                }
                ...
                try {
                    ...
                    System.out.println("  Web server on port "
                            + (5800 + display));
