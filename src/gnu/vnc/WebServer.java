...
public class WebServer implements Runnable {
    ...
    private class WebServerSocket implements Runnable {
        ...
        public WebServerSocket(Socket socket) throws IOException {
            ...
            new Thread(this, "VNCWebServerSocket-"
                    + socket.getInetAddress().getHostAddress()).start();
        }

        private void printApplet() {
            print.print("HTTP/1.0 200 Document follows\r\n" + "Server: "
                    + serverName + "\r\n" + "Content-Type: text/html\r\n"
                    + "\r\n" + "<!DOCTYPE HTML PUBLIC "
                    + "\"-//W3C//DTD HTML 3.2//EN\">\n" + "<HTML>\n"
                    + "<HEAD>\n" + "<TITLE>" + serverName + " [" + title
                    + "]</TITLE>\n" + "</HEAD>\n" + "\n" + "<BODY>\n"
                    + "<APPLET CODE=\"vncviewer.class\" HEIGHT=\"" + height
                    + "\" WIDTH=\"" + width + "\" ARCHIVE=\"vncviewer.jar\">"
        ...
        private void printNotFound() {
            print.print("HTTP/1.0 404 Not found\r\n"
                    + "Server: "
                    + serverName
                    + "\r\n"
                    + "Content-Type: text/html\r\n"
        ...
        private void processRequest(boolean post, String content)
                throws IOException {
            if (content.equals("/")) {
                printApplet();
                return;
                ...
                while (true) {
                    ...
                    if (line.length() == 0) {
                        ...
                        if (lineU.startsWith("GET")) {
                            ...
                            contentLength = Integer.parseInt(tokenizer
                                    .nextToken());
                        }
                    }
    ...
    private static String[] allowed = { "/vncviewer.jar",
            "/animatedMemoryImageSource.class", "/authenticationPanel.class",
            "/clipboardFrame.class", "/DesCipher.class", "/optionsFrame.class",
            "/rfbProto.class",
            "/vncCanvas.class",
            "/vncviewer.class",
            // MF viewer:
            "/ButtonPanel.class", "/optionsFrame$1.class",
            "/optionsFrame$2.class", "/optionsFrame$3.class",
            "/optionsFrame.class", "/TimeEvent.class", "/TimeOut.class",
            "/vncviewer$1.class" };
