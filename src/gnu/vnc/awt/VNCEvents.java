...
/**
 * Generates AWT events from RFB events for multiple RFB clients. Note that
 * because the event models do not correspond, it must manage an event state for
 * each client as a property
 **/
...
public class VNCEvents {
    ...
    private void fireKeyEvent(int id, int vk, int location, char character,
            int modifiers) {
        fireEvent(new KeyEvent(getFocusComponent(container), id,
                System.currentTimeMillis(), modifiers, vk, character, location));
    }

    private void fireMouseEvent(Component component, int id, int x, int y,
            int modifiers) {
    ...
    private void fireMouseEvent(Component component, int id, int x, int y,
            int modifiers, int button, int clicks) {
        fireEvent(new MouseEvent(component, id, System.currentTimeMillis(),
                modifiers, x, y, clicks, button == MouseEvent.BUTTON2, button));
    }
    ...
    private Point getLocation(Component component) {
        ...
        if (component == container) {
            ...
            if (parent != null) {
                ...
                return new Point(location.x + parentLocation.x, location.y
                        + parentLocation.y);
            } else {
                return location;
            }
    ...
    public void translateKeyEvent(RFBClient client, boolean down, int key) {
        ...
        if (down) {
            // Pressed
            fireKeyEvent(KeyEvent.KEY_PRESSED, vk[0], vk[1], character,
                    state.keyModifiers);
            state.keyPressed.add("" + vk[0]);
            if (character != KeyEvent.CHAR_UNDEFINED) {
                // Typed (for character keys only)
                fireKeyEvent(KeyEvent.KEY_TYPED, KeyEvent.VK_UNDEFINED,
                        KeyEvent.KEY_LOCATION_UNKNOWN, character,
                        state.keyModifiers);
            }
            ...
            // Released
            fireKeyEvent(KeyEvent.KEY_RELEASED, vk[0], vk[1], character,
                    state.keyModifiers);
        }
    }

    public void translatePointerEvent(RFBClient client, int buttonMask, int x,
            int y) {
        // Get state
        State state = getState(client);
        ...
        if ((buttonMask & rfb.Button1Mask) != 0)
            newMouseModifiers |= InputEvent.BUTTON1_DOWN_MASK;
        if ((buttonMask & rfb.Button2Mask) != 0)
            newMouseModifiers |= InputEvent.BUTTON2_DOWN_MASK;
        if ((buttonMask & rfb.Button3Mask) != 0)
            newMouseModifiers |= InputEvent.BUTTON3_DOWN_MASK;

        // Buttons
        if (newMouseModifiers != state.mouseModifiers) {
            // Button 1
            if (((newMouseModifiers & InputEvent.BUTTON1_DOWN_MASK) != 0)
                    && ((state.mouseModifiers & InputEvent.BUTTON1_DOWN_MASK) == 0)) {
                // Pressed
                fireMouseEvent(newComponent, MouseEvent.MOUSE_PRESSED, newX,
                        newY, state.keyModifiers | newMouseModifiers,
                        MouseEvent.BUTTON1);
                state.dragging = true;
            } else if (((newMouseModifiers & InputEvent.BUTTON1_DOWN_MASK) == 0)
                    && ((state.mouseModifiers & InputEvent.BUTTON1_DOWN_MASK) != 0)) {
                // Released
                fireMouseEvent(newComponent, MouseEvent.MOUSE_RELEASED, newX,
                        newY, state.keyModifiers | newMouseModifiers,
                        MouseEvent.BUTTON1);
                ...
                if (delta < 300) {
                    // Double-click
                    fireMouseEvent(newComponent, MouseEvent.MOUSE_CLICKED,
                            newX, newY, state.keyModifiers | newMouseModifiers,
                ...
                if (newComponent.isFocusable() && !newComponent.isFocusOwner()) {
                    ...
                    if (oldComponent != null) {
                        fireFocusEvent(oldComponent, FocusEvent.FOCUS_LOST,
                                newComponent);
                    }

                    // Gain new focus
                    newComponent.enableInputMethods(false); // native input methods will not work!
                    fireFocusEvent(newComponent, FocusEvent.FOCUS_GAINED,
                            oldComponent);
                }
            }

            // Button 2
            if (((newMouseModifiers & InputEvent.BUTTON2_DOWN_MASK) != 0)
                    && ((state.mouseModifiers & InputEvent.BUTTON2_DOWN_MASK) == 0)) {
                // Pressed
                fireMouseEvent(newComponent, MouseEvent.MOUSE_PRESSED, newX,
                        newY, state.keyModifiers | newMouseModifiers,
                        MouseEvent.BUTTON2);
            } else if (((newMouseModifiers & InputEvent.BUTTON2_DOWN_MASK) == 0)
                    && ((state.mouseModifiers & InputEvent.BUTTON2_DOWN_MASK) != 0)) {
                // Released
                fireMouseEvent(newComponent, MouseEvent.MOUSE_RELEASED, newX,
                        newY, state.keyModifiers | newMouseModifiers,
            ...
            // Button 3
            if (((newMouseModifiers & InputEvent.BUTTON3_DOWN_MASK) != 0)
                    && ((state.mouseModifiers & InputEvent.BUTTON3_DOWN_MASK) == 0)) {
                // Pressed
                fireMouseEvent(newComponent, MouseEvent.MOUSE_PRESSED, newX,
                        newY, state.keyModifiers | newMouseModifiers,
                        MouseEvent.BUTTON3);
            } else if (((newMouseModifiers & InputEvent.BUTTON3_DOWN_MASK) == 0)
                    && ((state.mouseModifiers & InputEvent.BUTTON3_DOWN_MASK) != 0)) {
                // Released
                fireMouseEvent(newComponent, MouseEvent.MOUSE_RELEASED, newX,
                        newY, state.keyModifiers | newMouseModifiers,
        ...
        if (newComponent != state.oldComponent) {
            if (state.oldComponent != null) {
                ...
                fireMouseEvent(state.oldComponent, MouseEvent.MOUSE_EXITED,
                        state.oldX, state.oldY, state.keyModifiers
                                | state.mouseModifiers);
            }

            // Entered new component
            fireMouseEvent(newComponent, MouseEvent.MOUSE_ENTERED, newX, newY,
                    state.keyModifiers | state.mouseModifiers);
        ...
        if ((newX != state.oldX) || (newY != state.oldY)) {
            ...
            if (state.dragging) {
                ...
                fireMouseEvent(newComponent, MouseEvent.MOUSE_DRAGGED, newX,
                        newY, state.keyModifiers | state.mouseModifiers);
            } else {
                // Moved (no button pressed)
                fireMouseEvent(newComponent, MouseEvent.MOUSE_MOVED, newX,
                        newY, state.keyModifiers | state.mouseModifiers);
            }
